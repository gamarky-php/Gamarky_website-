...
<head>
    ...
    <title>{{ __('حاسبة التكلفة - الاستيراد') }}</title>
    ...
    <div class="min-h-screen p-4">
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 mb-4">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ __('حاسبة تكلفة الاستيراد') }}</h1>
                    <p class="text-sm text-gray-500 mt-1">{{ __('احسب تكاليف الاستيراد بدقة مع هوامش الربح') }}</p>
                </div>
            ...
            <div x-show="refCode" class="mt-2">
                <span class="text-sm font-semibold text-blue-600">{{ __('رقم المرجع:') }} <span x-text="refCode"></span></span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 lg:col-span-2 space-y-4">
                ...
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <h3 class="font-semibold mb-3 text-gray-700">{{ __('سيناريوهات جاهزة') }}</h3>
                    <div class="space-y-2">
                ...
                <!-- Basic Inputs -->
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <h3 class="font-semibold mb-3 text-gray-700">{{ __('معلومات أساسية') }}</h3>
                    <div class="space-y-3">
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">{{ __('العملة') }}</label>
                        ...
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">{{ __('سعر الشراء') }}</label>
                        ...
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">{{ __('بلد المنشأ') }}</label>
                            <select x-model="inputs.origin_country" class="w-full text-sm border rounded px-2 py-1.5">
                                <option value="CN">{{ __('الصين') }}</option>
                                <option value="US">{{ __('أمريكا') }}</option>
                                <option value="DE">{{ __('ألمانيا') }}</option>
                                <option value="IT">{{ __('إيطاليا') }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">{{ __('ميناء الوصول') }}</label>
                            <select x-model="inputs.destination_port" class="w-full text-sm border rounded px-2 py-1.5">
                                <option value="EG-ALY">{{ __('الإسكندرية') }}</option>
                                <option value="SA-JED">{{ __('جدة') }}</option>
                                <option value="AE-DXB">{{ __('دبي') }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs text-gray-600 mb-1">{{ __('نوع الحاوية') }}</label>
            ...
            <div class="col-span-12 lg:col-span-10 space-y-4">
                ...
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="font-semibold text-gray-700">{{ __('بنود التكلفة') }}</h3>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-2 text-right">{{ __('البند') }}</th>
                                    <th class="px-4 py-2 text-right">{{ __('القيمة') }}</th>
                                    <th class="px-4 py-2 text-right">{{ __('النسبة %') }}</th>
                                    <th class="px-4 py-2 text-center">{{ __('إجراء') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr class="border-t">
                                        <td class="px-4 py-2">
                                            <select x-model="item.key" class="w-full border rounded px-2 py-1">
                                                <option value="international_freight">{{ __('شحن دولي') }}</option>
                                                <option value="insurance">{{ __('تأمين') }}</option>
                                                <option value="local_port_fees">{{ __('رسوم ميناء محلية') }}</option>
                                                <option value="customs_duties">{{ __('رسوم جمركية') }}</option>
                                                <option value="clearance_fees">{{ __('رسوم تخليص') }}</option>
                                                <option value="inland_transport">{{ __('نقل داخلي') }}</option>
                                            </select>
                                        </td>
                                        <td class="px-4 py-2">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <!-- Totals -->
                    <div class="bg-white rounded-lg shadow-sm p-4">
                        <h3 class="font-semibold mb-4 text-gray-700">{{ __('الإجماليات') }}</h3>
                        <div class="space-y-3">
                            ...
                            <div>
                                <label class="block text-xs text-gray-600 mb-1">{{ __('هامش الربح %') }}</label>
                                <input type="number" x-model="marginPercent" @input="calculate" 
                                       class="w-full border rounded px-2 py-1.5 text-sm">
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">{{ __('قيمة الهامش:') }}</span>
                                <span class="font-semibold text-green-600" x-text="formatCurrency(totals.margin_value)"></span>
                            </div>
                            <div class="flex justify-between pt-2 border-t">
                                <span class="text-gray-900 font-semibold">{{ __('سعر البيع:') }}</span>
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-4 lg:col-span-2">
                        <h3 class="font-semibold mb-4 text-gray-700">{{ __('المؤشرات الرئيسية') }}</h3>
                        <div class="grid grid-cols-3 gap-4">
                            <div class="text-center p-3 bg-blue-50 rounded">
                                <div class="text-2xl font-bold text-blue-600" x-text="kpis.duty_ratio + '%'"></div>
                                <div class="text-xs text-gray-600 mt-1">{{ __('نسبة الرسوم الجمركية') }}</div>
                            </div>
                            <div class="text-center p-3 bg-green-50 rounded">
                                <div class="text-2xl font-bold text-green-600" x-text="kpis.logistics_share + '%'"></div>
                                <div class="text-xs text-gray-600 mt-1">{{ __('حصة اللوجستيات') }}</div>
                            </div>
                            <div class="text-center p-3 bg-orange-50 rounded">
                                <div class="text-2xl font-bold text-orange-600" x-text="kpis.lead_time_days + ' ' + '{{ __('يوم') }}'"></div>
                                <div class="text-xs text-gray-600 mt-1">{{ __('وقت التسليم المتوقع') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function importCalculator() {
            return {
                ...
                saveScenario() {
                    alert('{{ __('تم حفظ السيناريو برقم:') }} ' + this.refCode);
                },
