...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
        <form wire:submit.prevent="trackShipment" class="flex gap-4">
            <div class="flex-1">
                <input type="text" wire:model="tracking_number" placeholder="{{ __('shipping.truck_tracker.tracking_placeholder') }}"
    ...
    @if($searchPerformed && $trackingData)
        <div class="grid md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-gradient-to-br from-teal-500 to-teal-600 text-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-sm font-bold">{{ __('shipping.truck_tracker.status') }}</h3>
            ...
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <h3 class="text-sm font-bold text-gray-800 mb-2">{{ __('shipping.truck_tracker.current_location') }}</h3>
            ...
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-sm font-bold text-gray-800">{{ __('shipping.truck_tracker.speed') }}</h3>
                    <i class="fas fa-tachometer-alt text-teal-500"></i>
                </div>
                <p class="text-2xl font-bold text-teal-600">{{ $trackingData['speed_kmh'] }} <span class="text-sm">{{ __('shipping.truck_tracker.kmh') }}</span></p>
            </div>

            {{-- ETA Card --}}
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <h3 class="text-sm font-bold text-gray-800 mb-2">{{ __('shipping.truck_tracker.estimated_arrival') }}</h3>
        ...
        <div class="grid md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                <h3 class="text-lg font-bold text-[#0F2E5D] mb-4 flex items-center">
                    ...
                    {{ __('shipping.truck_tracker.driver_info') }}
                </h3>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 text-sm">{{ __('shipping.truck_tracker.driver_name') }}</span>
                        <span class="font-semibold">{{ $trackingData['driver_name'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 text-sm">{{ __('shipping.truck_tracker.driver_phone') }}</span>
                        <a href="tel:{{ $trackingData['driver_phone'] }}" class="font-semibold text-teal-600 hover:underline">
                            {{ $trackingData['driver_phone'] }}
                        </a>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 text-sm">{{ __('shipping.truck_tracker.truck_plate') }}</span>
            ...
            <div class="md:col-span-2 bg-white rounded-xl shadow-lg p-6">
                <h3 class="text-lg font-bold text-[#0F2E5D] mb-4 flex items-center">
                    ...
                    {{ __('shipping.truck_tracker.live_map') }}
                </h3>
        ...
        {{-- PROGRESS BAR --}}
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            <h3 class="text-lg font-bold text-[#0F2E5D] mb-4">{{ __('shipping.truck_tracker.progress') }}</h3>
        ...
        {{-- EVENTS TIMELINE --}}
        <div class="bg-white rounded-xl shadow-lg p-6">
            <h3 class="text-lg font-bold text-[#0F2E5D] mb-6">{{ __('shipping.truck_tracker.journey_log') }}</h3>
            <div class="space-y-4">
