<div class="container mx-auto px-4 py-8" dir="rtl">
            ...
            @for($i = 1; $i <= $totalSteps; $i++)
                <div class="flex-1 flex flex-col items-center {{ $i < $totalSteps ? 'border-l border-gray-300' : '' }}">
                    ...
                    <span class="text-xs text-center {{ $currentStep >= $i ? 'text-teal-600 font-semibold' : 'text-gray-500' }}">
                        {{ ['تفاصيل الشحن', 'بيانات الحمولة', 'المستندات', 'التأمين والخدمات', 'الدفع', 'التأكيد'][$i-1] }}
                    </span>
                </div>
            @endfor
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        ...
        @if($currentStep == 1)
            <h2 class="text-2xl font-bold text-[#0F2E5D] mb-6">الخطوة 1: تفاصيل الشحن</h2>
            <div class="space-y-4">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">تاريخ التحميل *</label>
                        <input type="date" wire:model="loading_date"
                            class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-teal-500 text-right">
                        @error('loading_date') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">نوع البضاعة</label>
        ...
        @if($currentStep == 2)
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea wire:model="cargo_description" rows="4" placeholder="صف البضاعة بالتفصيل..."
        ...
        @if($currentStep == 3)
            <h2 class="text-2xl font-bold text-[#0F2E5D] mb-6">الخطوة 3: رفع المستندات</h2>
            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">الفاتورة التجارية (Invoice) *</label>
                    ...
                    @if($invoice_file)
                        <p class="text-sm text-green-600 mt-2"><i class="fas fa-check-circle"></i> تم الرفع: {{ $invoice_file->getClientOriginalName() }}</p>
                    @endif
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">قائمة التعبئة (Packing List) *</label>
                    ...
                    @if($packing_list_file)
                        <p class="text-sm text-green-600 mt-2"><i class="fas fa-check-circle"></i> تم الرفع: {{ $packing_list_file->getClientOriginalName() }}</p>
                    @endif
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">شهادة المنشأ (COO) - اختياري</label>
                    <input type="file" wire:model="coo_file" accept=".pdf,.jpg,.jpeg,.png"
                        class="w-full px-4 py-3 border-2 border-dashed rounded-lg focus:ring-2 focus:ring-teal-500">
                    @if($coo_file)
                        <p class="text-sm text-green-600 mt-2"><i class="fas fa-check-circle"></i> تم الرفع: {{ $coo_file->getClientOriginalName() }}</p>
                    @endif
                </div>
                ...
                <div wire:loading wire:target="invoice_file,packing_list_file,coo_file" class="text-center">
                    ...
                    <p class="text-gray-600 mt-2">جاري رفع الملف...</p>
                </div>
            </div>
        @endif
        ...
        @if($currentStep == 4)
            <h2 class="text-2xl font-bold text-[#0F2E5D] mb-6">الخطوة 4: التأمين والخدمات الإضافية</h2>
            <div class="space-y-4">
                <div class="border rounded-lg p-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" wire:model="needs_insurance" class="ml-3 w-5 h-5 text-teal-500">
                        <div>
                            <span class="font-semibold text-gray-800">تأمين شامل على البضاعة</span>
                            <p class="text-sm text-gray-600">يغطي التلف والفقدان والحوادث البحرية</p>
                        </div>
                    </label>
                    @if($needs_insurance)
                        <div class="mt-4">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">قيمة البضاعة (دولار) *</label>
                ...
                <div class="border rounded-lg p-4">
                    <label class="flex items-center cursor-pointer">
                        ...
                        <div>
                            <span class="font-semibold text-gray-800">خدمة التخليص الجمركي</span>
                            <p class="text-sm text-gray-600">نتولى إجراءات التخليص في ميناء الوصول</p>
                        </div>
                    </label>
                </div>
                ...
                <div class="border rounded-lg p-4">
                    <label class="flex items-center cursor-pointer">
                        ...
                        <div>
                            <span class="font-semibold text-gray-800">توصيل من الباب للباب</span>
                            <p class="text-sm text-gray-600">نقل البضاعة من مقر الشركة حتى مقر المستلم</p>
                        </div>
                    </label>
                </div>
            </div>
        ...
        @if($currentStep == 5)
            <h2 class="text-2xl font-bold text-[#0F2E5D] mb-6">الخطوة 5: طريقة الدفع</h2>
            <div class="space-y-4">
                <div class="border rounded-lg p-4 cursor-pointer {{ $payment_method == 'bank_transfer' ? 'border-teal-500 bg-teal-50' : '' }}"
                    ...
                    <label class="flex items-center cursor-pointer">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">سيتم إرسال تفاصيل الحساب بعد التأكيد</p>
                        </div>
                    </label>
                </div>
                ...
                <div class="border rounded-lg p-4 cursor-pointer {{ $payment_method == 'credit_card' ? 'border-teal-500 bg-teal-50' : '' }}"
                    ...
                    <label class="flex items-center cursor-pointer">
                        ...
                        <div>
                            <span class="font-semibold text-gray-800">بطاقة ائتمان</span>
                            <p class="text-sm text-gray-600">دفع آمن عبر بوابة الدفع</p>
                        </div>
                    </label>
                </div>
                ...
                <div class="border rounded-lg p-4 cursor-pointer {{ $payment_method == 'cod' ? 'border-teal-500 bg-teal-50' : '' }}"
                    ...
                    <label class="flex items-center cursor-pointer">
                        ...
                        <div>
                            <span class="font-semibold text-gray-800">دفع عند الاستلام</span>
                            <p class="text-sm text-gray-600">الدفع عند استلام البضاعة في الميناء</p>
                        </div>
                    </label>
                </div>
            </div>
        ...
        {{-- STEP 6: Confirmation --}}
        @if($currentStep == 6)
            <h2 class="text-2xl font-bold text-[#0F2E5D] mb-6">الخطوة 6: مراجعة وتأكيد الطلب</h2>
            ...
            <div class="bg-gray-50 rounded-lg p-6 mb-6 space-y-3">
                ...
                <div class="flex justify-between border-b pb-2">
                    <span class="text-gray-600">الوزن/الحجم:</span>
                    <span class="font-semibold">{{ $weight_kg }} كجم / {{ $cbm }} م³</span>
                </div>
                <div class="flex justify-between border-b pb-2">
                ...
                <div class="flex justify-between">
                    <span class="text-gray-600">خدمات إضافية:</span>
                    <span class="font-semibold">
                        {{ $needs_insurance ? 'تأمين • ' : '' }}
                        {{ $needs_customs_clearance ? 'تخليص • ' : '' }}
                        {{ $needs_door_delivery ? 'توصيل' : '' }}
                    </span>
                </div>
            </div>
            ...
            <div class="border rounded-lg p-4 mb-6">
                <label class="flex items-start cursor-pointer">
                    ...
                    <div>
                        <span class="font-semibold text-gray-800">أوافق على الشروط والأحكام *</span>
                        <p class="text-sm text-gray-600 mt-1">
                            بقبولك لهذه الشروط، فإنك توافق على سياسة الخصوصية وشروط الاستخدام الخاصة بالمنصة
                        </p>
                    </div>
                </label>
