...
<div class="rounded-xl bg-white shadow-sm p-4 md:p-5">
    <div class="flex items-center justify-between mb-3">
        <h3 class="text-lg font-semibold">{{ __('dashboard.shared.recommended_suppliers.title') }}</h3>
        @if (Route::has('admin.suppliers.index'))
            <a href="{{ route('admin.suppliers.index') }}" class="text-sm text-blue-600 hover:underline">{{ __('dashboard.shared.recommended_suppliers.view_all') }}</a>
        @endif
    </div>

    @if($suppliers->isEmpty())
        <div class="text-sm text-slate-500">{{ __('dashboard.shared.recommended_suppliers.empty') }}</div>
    @else
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        @if($waLink)
                            <a href="{{ $waLink }}" target="_blank" rel="noopener noreferrer" class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-100 text-sm">
                                ...
                                {{ __('dashboard.shared.recommended_suppliers.whatsapp') }}
                            </a>
                        @else
                            <span class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-md bg-slate-50 text-slate-400 border border-slate-100 text-sm">{{ __('dashboard.shared.recommended_suppliers.whatsapp') }}</span>
                        @endif
                        ...
                        @if($callNumber)
                            <a href="tel:{{ $callNumber }}" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-slate-50 text-slate-700 border border-slate-100 text-sm">
                                ...
                                {{ __('dashboard.shared.recommended_suppliers.call') }}
                            </a>
                        @else
                            <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-slate-50 text-slate-400 border border-slate-100 text-sm">{{ __('dashboard.shared.recommended_suppliers.call') }}</span>
                        @endif
                    </div>
                </div>
            @endforeach
