...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg shadow-lg p-6 text-white">
        <h2 class="text-2xl font-bold">{{ __('📊 حاسبة عمولات الوكالة') }}</h2>
        <p class="text-indigo-100 mt-2">{{ __('احسب رسوم التأسيس والعمولات ومواد التسويق لوكلاء الشحن والعلامات التجارية') }}</p>
    </div>

    {{-- Agency Type Selector --}}
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">{{ __('نوع الوكالة') }}</h3>
        <div class="flex gap-4">
            <button
                ...
                <div class="flex items-center justify-center gap-2">
                    ...
                    <span class="font-semibold text-lg">{{ __('وكيل شحن') }}</span>
                </div>
            </button>
            ...
            <button
                ...
                <div class="flex items-center justify-center gap-2">
                    ...
                    <span class="font-semibold text-lg">{{ __('وكيل علامة تجارية') }}</span>
                </div>
            </button>
        </div>
    </div>
    ...
    @if($agencyType === 'shipping')
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('قيمة الشحنة ($)') }}</label>
                        <input type="number" wire:model.live="shipment_value" min="0" step="100"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('نسبة العمولة (%)') }}</label>
                        <input type="number" wire:model.live="commission_percentage" min="0" max="100" step="0.5"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('رسوم التأسيس ($)') }}</label>
                        <input type="number" wire:model.live="setup_fee" min="0" step="50"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('الحد الأدنى للعمولة ($)') }}</label>
                        <input type="number" wire:model.live="minimum_commission" min="0" step="50"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('عدد الشحنات') }}</label>
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-gray-700">{{ __('مواد تسويقية') }}</span>
                        </label>
                        
                        @if($marketing_materials)
                            <div class="mt-3 mr-8">
                                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('تكلفة المواد التسويقية ($)') }}</label>
                ...
                <div class="bg-blue-50 rounded-lg border border-blue-200 p-4">
                    <div class="flex gap-3">
                        ...
                        <div class="text-sm text-blue-800">
                            <p class="font-semibold mb-1">{{ __('ملاحظة') }}</p>
                            <p>{{ __('يتم تطبيق الحد الأدنى للعمولة تلقائياً إذا كانت العمولة المحسوبة أقل من الحد المحدد.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    @if($agencyType === 'brand')
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('المبيعات السنوية المتوقعة ($)') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('الحد الأدنى السنوي ($)') }}</label>
                        <input type="number" wire:model.live="brand_minimum_annual" min="0" step="500"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('باقة التسويق') }}</label>
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-gray-700">{{ __('حصرية إقليمية') }}</span>
                        </label>
                        
                        @if($territory_exclusivity)
                            <div class="mt-3 mr-8">
                                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('رسوم الحصرية ($)') }}</label>
            ...
            <div class="space-y-4">
                <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
                    <p class="text-sm font-medium text-purple-100">{{ __('إجمالي التكلفة السنوية') }}</p>
                    <p class="text-4xl font-bold mt-2">${{ number_format($results['grand_total'] ?? 0, 2) }}</p>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                            <span class="text-sm text-gray-600">{{ __('عمولة أساسية') }}</span>
                            <span class="text-sm font-semibold text-gray-900">${{ number_format($results['base_commission'] ?? 0, 2) }}</span>
                        </div>

                        <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                            <span class="text-sm text-gray-600">{{ __('العمولة النهائية (بعد الحد الأدنى)') }}</span>
                            <span class="text-sm font-semibold text-purple-600">${{ number_format($results['final_commission'] ?? 0, 2) }}</span>
                        </div>

                        <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                            <span class="text-sm text-gray-600">{{ __('باقة التسويق') }} ({{ $results['marketing_package'] ?? '' }})</span>
                            <span class="text-sm font-semibold text-gray-900">${{ number_format($results['marketing_cost'] ?? 0, 2) }}</span>
                        </div>

                        @if($territory_exclusivity)
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-sm text-gray-600">{{ __('رسوم الحصرية الإقليمية') }}</span>
                ...
                <div class="bg-purple-50 rounded-lg border border-purple-200 p-4">
                    <div class="flex gap-3">
                        ...
                        <div class="text-sm text-purple-800">
                            <p class="font-semibold mb-1">{{ __('ملاحظة') }}</p>
                            <p>{{ __('يتم تطبيق الحد الأدنى للعمولة السنوية تلقائياً. الحصرية الإقليمية توفر حقوق حصرية للوكيل في المنطقة المحددة.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
