    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('بحث المستخلصين الجمركيين') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('ابحث عن أفضل المستخلصين حسب البلد والميناء والخبرة والتقييم') }}</p>
                </div>
                <button wire:click="$toggle('showFilters')" 
                    ...
                    {{ $showFilters ? __('إخفاء') : __('إظهار') }} {{ __('الفلاتر') }}
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('إجمالي المستخلصين') }}</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('متوسط التقييم') }}</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('دول التغطية') }}</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('تقييمات جديدة (30 يوم)') }}</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-8 border border-gray-200 dark:border-gray-700">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div class="md:col-span-2">
                           <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('البحث بالاسم') }}</label>
                        <input type="text" wire:model.live.debounce.500ms="searchTerm"
                               placeholder="{{ __('اسم المستخلص أو الشركة...') }}"
                    ...
                    {{-- Country Filter --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('البلد') }}</label>
                    ...
                    {{-- Port Filter --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('الميناء') }}</label>
                    ...
                    {{-- Activity Filter --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('النشاط') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('الحد الأدنى للخبرة') }} ({{ $minExperience }} {{ __('سنة') }})
                        </label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('الحد الأدنى للتقييم') }} ({{ $minScore }}/5)
                        </label>
                    ...
                    {{-- Sort By --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('الترتيب حسب') }}</label>
                        <select wire:model.live="sortBy"
                                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                            <option value="score">{{ __('التقييم') }}</option>
                            <option value="experience_years">{{ __('الخبرة') }}</option>
                            <option value="name">{{ __('الاسم') }}</option>
                        </select>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
                    <div class="p-6">
                        ...
                        <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-3">
                            ...
                            <span>{{ $broker->experience_years }} {{ __('سنة خبرة') }}</span>
                        </div>
                        ...
                        @if($broker->ports)
                            ...
                            @if(count($portsArray) > 0)
                                <div class="mb-3">
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">{{ __('الموانئ المغطاة:') }}</p>
                        ...
                        @if($broker->activities)
                            ...
                            @if(count($activitiesArray) > 0)
                                <div class="mb-4">
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">{{ __('الأنشطة:') }}</p>
                                    <div class="flex flex-wrap gap-1">
                        ...
                        @if($broker->certifications)
                            ...
                            @if(count($certs) > 0)
                                <div class="mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">{{ __('الشهادات:') }}</p>
                                    <div class="flex flex-wrap gap-1">
            ...
            @empty
                <div class="col-span-full bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center border border-gray-200 dark:border-gray-700">
                    ...
                    <p class="text-lg text-gray-600 dark:text-gray-400 mb-2">{{ __('لم يتم العثور على مستخلصين') }}</p>
                    <p class="text-sm text-gray-500 dark:text-gray-500">{{ __('جرب تعديل معايير البحث') }}</p>
                    <button wire:click="resetFilters" 
    ...
    <div wire:loading.flex class="fixed inset-0 bg-gray-900/50 items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-xl">
            <div class="flex items-center space-x-4 space-x-reverse">
                ...
                <span class="text-gray-900 dark:text-white font-medium">{{ __('جاري البحث...') }}</span>
            </div>
        </div>
    </div>
</div>
