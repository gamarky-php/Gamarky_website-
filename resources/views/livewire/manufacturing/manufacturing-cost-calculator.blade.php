    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('حاسبة تكاليف التصنيع') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('احسب تكلفة الإنتاج وسعر البيع المثالي') }}</p>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">{{ __('معلومات المنتج') }}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" wire:model.blur="product_name"
                                   placeholder="{{ __('مثال: مكونات إلكترونية') }}"
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {{ __('حجم الدفعة (وحدات)') }} <span class="text-red-500">*</span>
                            </label>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('التكاليف المباشرة') }}</h2>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('مواد خام، عمالة مباشرة، مواد مساعدة') }}</p>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tfoot class="bg-blue-50 dark:bg-blue-900/20">
                                <tr>
                                    <td class="px-6 py-4 text-right font-semibold text-gray-900 dark:text-white">{{ __('إجمالي التكاليف المباشرة:') }}</td>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('التكاليف غير المباشرة') }}</h2>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('طاقة، صيانة، استهلاك معدات، تكاليف إدارية') }}</p>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tfoot class="bg-purple-50 dark:bg-purple-900/20">
                                <tr>
                                    <td class="px-6 py-4 text-right font-semibold text-gray-900 dark:text-white">{{ __('إجمالي التكاليف غير المباشرة:') }}</td>
                ...
                <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-800/20 rounded-xl p-6 border border-indigo-200 dark:border-indigo-700">
                    <h3 class="text-lg font-bold text-indigo-900 dark:text-indigo-300 mb-4">{{ __('ملخص التكاليف') }}</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between items-center pb-3 border-b border-indigo-200 dark:border-indigo-700">
                            <span class="text-gray-700 dark:text-gray-300">{{ __('التكاليف المباشرة:') }}</span>
                            <span class="font-semibold text-blue-600 dark:text-blue-400">${{ number_format($total_direct_cost, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center pb-3 border-b border-indigo-200 dark:border-indigo-700">
                            <span class="text-gray-700 dark:text-gray-300">{{ __('التكاليف غير المباشرة:') }}</span>
                        ...
                        <div class="flex justify-between items-center pb-3 border-b-2 border-indigo-300 dark:border-indigo-600">
                            <span class="font-bold text-gray-900 dark:text-white">{{ __('إجمالي تكلفة الإنتاج:') }}</span>
                        ...
                        <div class="flex justify-between items-center pt-3 border-t-2 border-green-500">
                            <span class="font-bold text-lg text-gray-900 dark:text-white">{{ __('سعر البيع المقترح:') }}</span>
                        ...
                        <div class="flex justify-between items-center text-sm bg-white/50 dark:bg-gray-800/50 p-3 rounded-lg">
                            <span class="text-gray-600 dark:text-gray-400">{{ __('تكلفة الوحدة') }} ({{ $batch_size }} {{ __('وحدة') }}):</span>
                            <span class="font-semibold text-gray-900 dark:text-white">${{ number_format($cost_per_unit, 2) }} / {{ __('وحدة') }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm bg-white/50 dark:bg-gray-800/50 p-3 rounded-lg">
                            <span class="text-gray-600 dark:text-gray-400">{{ __('سعر الوحدة للبيع:') }}</span>
                            <span class="font-semibold text-green-600 dark:text-green-400">${{ number_format($unit_price, 2) }} / {{ __('وحدة') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                            <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">{{ __('نسبة التكاليف المباشرة') }}</p>
                            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $direct_cost_ratio }}%</p>
                        </div>

                        {{-- Indirect Cost Ratio --}}
                        <div class="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                            <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">{{ __('نسبة التكاليف غير المباشرة') }}</p>
                        ...
                        <div class="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                            <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">{{ __('نقطة التعادل (وحدات)') }}</p>
                ...
                <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-800/20 rounded-xl p-6 border border-indigo-200 dark:border-indigo-700">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h4 class="font-semibold text-indigo-900 dark:text-indigo-300 mb-2">{{ __('نصيحة') }}</h4>
                            <p class="text-sm text-indigo-800 dark:text-indigo-400">
                                {{ __('نقطة التعادل تساعدك في تحديد الحد الأدنى من الوحدات التي يجب إنتاجها لتغطية التكاليف الثابتة.') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="calculationName"
                               placeholder="{{ __('مثال: تكلفة إنتاج منتج X - يناير 2025') }}"
                               class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
