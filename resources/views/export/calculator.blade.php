@extends('layouts.app')
@section('title', 'حاسبة تكاليف التصدير')
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="lg:w-[85%] bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-6 bg-gradient-to-r from-blue-600 to-purple-600">
          <h1 class="text-3xl font-bold text-white text-center">حاسبة تكاليف التصدير</h1>
          <p class="text-white/90 text-center mt-2">احسب تكاليف الشحن الدولي حسب الإنكوترمز</p>
        </div>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-indigo-500 to-blue-500">
                  <tr>
                    ...
                    <th class="sticky right-[4rem] z-20 bg-gradient-to-r from-indigo-600 to-blue-600 px-6 py-3 text-center text-sm font-bold text-white min-w-[200px]">البيان</th>
                    ...
                    <th class="px-4 py-3 text-center">
                      <button type="button" @click="addItem" class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white text-xs rounded-lg">+ بند</button>
                    </th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-100">
                  ...
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="sticky right-[4rem] bg-white px-6 py-3 text-sm font-semibold text-gray-800">الوصف</td>
                    <template x-for="(item, idx) in items" :key="idx">
                      <td class="px-4 py-3">
                        <input type="text" x-model="item.description" :name="`items[${idx}][description]`" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="وصف البند">
                      </td>
                    </template>
                    <td></td>
                  </tr>
                  ...
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="sticky right-[4rem] bg-white px-6 py-3 text-sm font-semibold text-gray-800">الكمية</td>
                    <template x-for="(item, idx) in items" :key="idx">
                      <td class="px-4 py-3">
                  ...
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="sticky right-[4rem] bg-white px-6 py-3 text-sm font-semibold text-gray-800">سعر الوحدة</td>
                    <template x-for="(item, idx) in items" :key="idx">
                      <td class="px-4 py-3">
                  ...
                  <tr class="bg-yellow-50 font-semibold">
                    ...
                    <td class="sticky right-[4rem] bg-yellow-50 px-6 py-3 text-sm font-bold text-gray-900">تكلفة EXW</td>
                    <template x-for="(item, idx) in items" :key="idx">
                  ...
                  <tr class="hover:bg-gray-50" x-show="['FOB','CFR','CIF'].includes(incoterm)">
                    ...
                    <td class="sticky right-[4rem] bg-white px-6 py-3 text-sm font-semibold text-gray-800">شحن داخلي</td>
                    <template x-for="(item, idx) in items" :key="idx">
                      <td class="px-4 py-3">
                  ...
                  <tr class="bg-green-50 font-semibold" x-show="['FOB','CFR','CIF'].includes(incoterm)">
                    ...
                    <td class="sticky right-[4rem] bg-green-50 px-6 py-3 text-sm font-bold text-gray-900">تكلفة FOB</td>
                    <template x-for="(item, idx) in items" :key="idx">
                  ...
                  <tr class="hover:bg-gray-50" x-show="['CFR','CIF'].includes(incoterm)">
                    ...
                    <td class="sticky right-[4rem] bg-white px-6 py-3 text-sm font-semibold text-gray-800">شحن دولي</td>
                    <template x-for="(item, idx) in items" :key="idx">
                      <td class="px-4 py-3">
                  ...
                  <tr class="bg-purple-50 font-semibold" x-show="['CFR','CIF'].includes(incoterm)">
                    ...
                    <td class="sticky right-[4rem] bg-purple-50 px-6 py-3 text-sm font-bold text-gray-900">تكلفة CFR</td>
                    <template x-for="(item, idx) in items" :key="idx">
                  ...
                  <tr class="hover:bg-gray-50" x-show="incoterm==='CIF'">
                    ...
                    <td class="sticky right-[4rem] bg-white px-6 py-3 text-sm font-semibold text-gray-800">التأمين</td>
                    <template x-for="(item, idx) in items" :key="idx">
                      <td class="px-4 py-3">
                  ...
                  <tr class="bg-red-50 font-semibold" x-show="incoterm==='CIF'">
                    ...
                    <td class="sticky right-[4rem] bg-red-50 px-6 py-3 text-sm font-bold text-gray-900">تكلفة CIF</td>
                    <template x-for="(item, idx) in items" :key="idx">
      ...
      <div class="lg:w-[15%] lg:sticky lg:top-4 lg:self-start bg-white rounded-xl shadow-lg p-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">مدخلات الشحنة</h2>

        <div class="space-y-4">
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">الإنكوترمز</label>
          ...
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">مرجع الشحنة</label>
            <input type="text" name="reference" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="اختياري">
          </div>

          <div class="pt-4 border-t border-gray-200">
            <h3 class="text-sm font-bold text-gray-700 mb-2">الإجمالي</h3>
